...
-- Corrected DATEADD issue with proper datetime casting
                ...
                CASE 
                    ...
                    WHEN EXISTS (
                        ...
                        WHERE w2.WeekStart = w.WeekStart
                          ...
                          AND w2.Timestamp > DATEADD(hour, 1, CONVERT(DATETIME, w.WeekStart))
                ...
                CASE 
                    WHEN EXISTS (
                        ...
                        WHERE s2.WeekStart = s4.WeekStart
                          ...
                          AND s2.Timestamp > DATEADD(hour, 1, CONVERT(DATETIME, s4.WeekStart))
...
SELECT 
    ...
    MarginLow,
    WeeklyBias,
    -- Fixed: Convert to datetime properly before adding hours
    DATEADD(hour, 9, DATEADD(minute, 15, CONVERT(DATETIME, WeekStart))) as FirstHourTime,
